...
help:
	...
	@echo 'initialize_development - migrate and load Admissions development fixtures'
...
initialize_development:
	@set -e; \
		docker compose up -d; \
		attempt=1; \
		until poetry run python manage.py initialize_development; do \
			if [ "$$attempt" -ge 10 ]; then \
				echo "Development initialization failed after $$attempt attempts."; \
				exit 1; \
			fi; \
			echo "Development initialization failed; retrying in 2 seconds... ($$attempt/10)"; \
			sleep 2; \
			attempt=$$((attempt + 1)); \
		done
...
.PHONY: ci_settings dev_settings initialize_development cypress_fixtures dev

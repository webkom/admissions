...
export const buildManualBlocksByDay = ({
  dates,
  dayStartMinute,
  dayEndMinute,
  sessionDuration,
  ...
  dates.flatMap((date) => {
    ...
    for (
      ...
      minute + sessionDuration <= dayEndMinute;
      minute += sessionDuration
    ) {
      slots.push(makeSlotKey(date, minute));
    }
    return slots.length > 0 ? [{ slots }] : [];
  });

export interface ManualBlockCoordinateSystem {
  dates: string[];
  dayStartMinute: number;
  dayEndMinute: number;
  sessionDuration: number;
...
export const manualBlocksRequireReset = (
  previous: ManualBlockCoordinateSystem,
  next: ManualBlockCoordinateSystem,
) =>
  JSON.stringify(previous.dates) !== JSON.stringify(next.dates) ||
  previous.dayStartMinute !== next.dayStartMinute ||
  previous.dayEndMinute !== next.dayEndMinute ||

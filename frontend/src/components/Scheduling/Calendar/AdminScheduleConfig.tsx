...
import ManualScheduleBlockEditor from "./ManualScheduleBlockEditor";
...
const AdminScheduleConfig: React.FC<AdminScheduleConfigProps> = ({
  ...
  const [draftBlockMode, setDraftBlockMode] =
    useState<ScheduleBlockMode>(blockMode);
  const [draftManualBlocks, setDraftManualBlocks] =
    useState<ManualScheduleBlock[]>(manualBlocks);
  const [baselineBlockMode, setBaselineBlockMode] = useState(blockMode);
  const [baselineManualBlocks, setBaselineManualBlocks] =
    useState(manualBlocks);
  ...
  useEffect(() => {
    ...
    if (
      draftBlockMode === "manual" &&
      manualBlocksRequireReset(
        previous.manualBlockCoordinates,
        gridShape.manualBlockCoordinates,
      )
    ) {
      setDraftManualBlocks(
        buildManualBlocksByDay({
          dates,
          dayStartMinute: startMinute,
          dayEndMinute: endMinute,
  ...
  const hasPendingChanges =
    configurationHasPendingChanges ||
    layoutResetRequested ||
    draftBlockMode !== baselineBlockMode ||
    JSON.stringify(draftManualBlocks) !== JSON.stringify(baselineManualBlocks);
  const proposalInvalidatingChange =
    modelProposalInvalidatingChange ||
    layoutResetRequested ||
    draftBlockMode !== baselineBlockMode ||
    JSON.stringify(draftManualBlocks) !== JSON.stringify(baselineManualBlocks);
  ...
  const applyIncomingConfiguration = useCallback(() => {
    ...
    setDraftManualBlocks(manualBlocks);
    setBaselineBlockMode(blockMode);
    setBaselineManualBlocks(manualBlocks);
  }, [
    blockMode,
    chunkBreakMinutes,
    chunkSize,
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        slotOverrides:
          draftBlockMode === "manual"
            ...
            : layoutVersion >= 2 || layoutResetRequested
              ...
              : undefined,
        blockMode: draftBlockMode,
        ...(draftBlockMode === "manual"
          ? { manualBlocks: draftManualBlocks }
          : {}),
      ...
      setBaselineBlockMode(draftBlockMode);
      setBaselineManualBlocks(draftManualBlocks);
      setRemoteRevisionChanged(false);
      ...
      >
        ...
        <SchedulePanelBody className="space-y-6">
          ...
          >
            ...
            <fieldset className="mb-4 rounded-lg border border-border bg-surface-muted p-4">
              <legend className="px-1 text-detail font-bold text-text-primary">
                Blokkstruktur
              </legend>
              <p className="mb-3 mt-0 text-detail text-text-muted">
                Standardblokker følger pausemønsteret. Manuelle blokker lar deg
                definere nøyaktige, sammenhengende grupper av tidsluker.
              </p>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  aria-pressed={draftBlockMode === "standard"}
                  className={cn(
                    actionButtonBase,
                    actionButtonGhost,
                    "px-3 py-1.5",
                    draftBlockMode === "standard" &&
                      "border-brand-border bg-brand-soft text-brand",
                  )}
                  disabled={isSaving}
                  onClick={() => setDraftBlockMode("standard")}
                >
                  Standardblokker
                </button>
                <button
                  type="button"
                  aria-pressed={draftBlockMode === "manual"}
                  className={cn(
                    actionButtonBase,
                    actionButtonGhost,
                    "px-3 py-1.5",
                  ...
                  disabled={isSaving || isInvalidRange || dates.length === 0}
                  onClick={() => {
                    setDraftBlockMode("manual");
                    if (draftManualBlocks.length === 0) {
                      setDraftManualBlocks(
                        buildManualBlocksByDay({
                          dates,
                          dayStartMinute: startMinute,
            ...
            {draftBlockMode === "manual" && (
              <div className="mb-4">
                <ManualScheduleBlockEditor
                  dates={dates}
                  dayStartMinute={startMinute}
                  dayEndMinute={endMinute}
                  sessionDuration={pendingDuration}
                  value={draftManualBlocks}
                  disabled={isSaving || isInvalidRange || remoteRevisionChanged}
                  onChange={setDraftManualBlocks}
                />
              </div>
            )}
            {layoutVersion < 2 && !layoutResetRequested && (

...
export function useSolveJob(admissionSlug: string) {
  ...
  const cancelInBackground = useCallback(
    (jobId: string) => {
      void lifecycle.cancel(jobId).catch(() => undefined);
    },
    [lifecycle],
  );
  ...
  const reset = useCallback(() => {
    ...
    clearStoredProposal();
    if (jobId) {
      cancelInBackground(jobId);
    }
  }, [cancelInBackground, clearStoredJob, clearStoredProposal]);
      ...
      try {
        ...
        if (!storedJob) {
          ...
          if (initialJob.preview_only) {
            cancelInBackground(initialJob.job_id);
            return;
          }
          storedJob = {
        ...
        const restoreFinishedJob = (finishedJob: SolveJob) => {
          if (restoredJob.mode === "preview" || finishedJob.preview_only) {
            cancelInBackground(finishedJob.job_id);
            return;
          }
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (solveRunRef.current !== runId) return;
        setError(
          "Kunne ikke gjenopprette forslaget. Last siden på nytt for å prøve igjen.",
        );
      }
    })();
  }, [

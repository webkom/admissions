...
export interface AdmissionGroupContext {
  group: {
    id: string;
    name: string;
  };
  membership_role: "leader" | "recruiting" | "member";
  membership_roles: Array<"leader" | "recruiting" | "member">;
  sources: {
    admission_group: boolean;
    admin_group: boolean;
  };
  actions: {
    open_member_workspace: boolean;
    administer_group_applications: boolean;
  };
}

export interface AdmissionActions {
  administer_all_applications: boolean;
  administer_schedule: boolean;
  authority_group_ids: string[];
}

export interface AdmissionResourceScopes {
  schedule: "admission";
  availability: "admission_user";
}

export interface AdmissionUserData {
  ...
  group_contexts?: AdmissionGroupContext[];
  admission_actions?: AdmissionActions;
  resource_scopes?: AdmissionResourceScopes;
}

...
import SchedulePageAccessBoundary, {
  type ScheduleWorkspaceProps,
} from "./SchedulePageAccessBoundary";
import FoundationCoverageWorkspace from "./FoundationCoverageWorkspace";

const SchedulePage: React.FC = () => {
  return (
    <SchedulePageAccessBoundary>
      {(props, workspaceKey) => (
        <CommonScheduleView key={workspaceKey} {...props} />
      )}
    </SchedulePageAccessBoundary>
  );
};

const CommonScheduleView: React.FC<ScheduleWorkspaceProps> = (props) => {
...
interface LoadedScheduleViewProps extends ScheduleWorkspaceProps {
...
const LoadedScheduleView: React.FC<LoadedScheduleViewProps> = ({
  ...
  const conflictCollection = deriveConflictCollectionState({
    savedSchedule,
    candidateIds: interviewCandidates?.map((candidate) => candidate.id),
    participants: availabilityParticipants,
  });
  const conflictCollectionTaskRequired = requiresConflictCollectionTask({
    savedSchedule,
  ...
  const workflow = useScheduleWorkflow({
    ...
    conflictCollectionNeedsAction: conflictCollectionTaskRequired,
    conflictCollectionReady:
      Boolean(savedSchedule?.is_distributed) ||
      !savedSchedule?.schedule.length ||
      conflictCollection.complete,
  });
    ...
    <div className="mx-auto w-full max-w-6xl px-5 pb-20 pt-8 handheld:px-4">
      ...
      <main className="mt-3 flex flex-col gap-3">
        ...
        {isAdmin && visitedSections.has("config") && (
          <div className={activeSection === "config" ? "" : "hidden"}>
            ...
            <FoundationCoverageWorkspace
              ...
              dayStartMinute={dayStartMinute}
              dayEndMinute={dayEndMinute}
              coverageReady={foundationStage.kind === "coverage_ready"}
              submittedCount={submittedAvailabilityCount}
              participantCount={activeAvailabilityParticipants.length}
              ...
              onCreateDraft={() => handleSectionChange("solver")}
            />
          </div>
        )}
        ...
        {isAdmin && visitedSections.has("solver") && (
          ...
          >
            {conflictCollectionTaskRequired ? (
              <ConflictCollectionPanel
                open={Boolean(savedSchedule?.conflict_collection_open)}
                participantCount={conflictCollection.participantCount}
                completedCount={conflictCollection.completedCount}
                candidateCount={interviewCandidates?.length ?? 0}
                saving={conflictCollectionSaving}
                stale={conflictCollection.stale}
                ...
                >
                  <SolverView
                    ...
                    missingReviewerNames={
                      publicationReadiness.missingReviewerNames
                    }
        ...
        {activeSection === "plan" &&
          (savedSchedule?.is_distributed ? (
            <DistributedPlanView
              ...
              committeeScopeId={committeeScopeId}
              savedSchedule={savedSchedule}
              dates={dates}
              isAdmin={isAdmin}

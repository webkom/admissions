...
const LandingPageSkeleton: React.FC<PropsWithChildren> = ({ children }) => {
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col items-center px-8 py-16 handheld:px-4 handheld:py-8">
      <div className="mb-8 max-w-44 transition-transform duration-200 hover:scale-[1.02] handheld:max-w-36">
        <AbakusLogo />
      </div>
      <h1 className="mb-4 text-center text-5xl font-extrabold tracking-tight text-text-strong handheld:text-display-lg">
        Opptak
      </h1>
      {children}
      {isManager() && (

describe("interview plan workflow", () => {
  ...
  it("moves focus through outreach actions and restores the trigger", () => {
    cy.get('[aria-label^="Intervjustatus for "]')
      .first()
      .invoke("text")
      .then((statusText) => {
        const actionLabel =
          statusText.trim() === "Ikke kalt inn"
            ? "Send innkalling"
            : "Send påminnelse";
        cy.contains("button", actionLabel)
          .as("outreachTrigger")
          .focus()
          .click();
      });

    cy.contains('[role="menuitem"]', "Åpne SMS-utkast").should("be.focused");
    cy.focused().type("{downarrow}");
    cy.contains('[role="menuitem"]', "Kopier meldingstekst").should(
      "be.focused",
    );
    ...
    cy.contains('[role="menuitem"]', "Åpne SMS-utkast")
      .should("be.focused")
      .type("{esc}");
    cy.get("@outreachTrigger").should("be.focused");
  });

  it("restores focus and confirms copied outreach text", () => {
    ...
    cy.contains('[role="menuitem"]', "Kopier meldingstekst").click();
    cy.get("@outreachTrigger").should("be.focused");
    cy.contains('[role="status"]', "Kopiert").should("be.visible");
  });
  ...
  it("keeps an authorized actor's custom template across reloads", () => {
    const customMessage = "Hei! Egen testmal.";
    cy.contains("summary", "Meldingsmal").click();
    cy.get("#interview-outreach-sms-body").clear().type(customMessage);
    cy.contains("Lagret i denne nettleseren").should("be.visible");

    cy.intercept(
      "GET",
      "/api/admin/admission/webkom-open/availability/",
      (request) => {
    ...
    cy.get("#interview-outreach-sms-body").should("have.value", customMessage);
  });

...
const mountGrids = () => {
  visitStaticFixture("selectable-schedule-grid");
...
describe("shared selectable schedule grid", () => {
  ...
  it("excludes opted-out interviewers from solver participants", () => {
    cy.get("[data-cy=schedule-participants-counts]").should("have.text", "2/1");
  });
  ...
  it("keeps availability changes retryable without an unhandled rejection", () => {
    collectUnhandledRejections().as("unhandledRejections");
    cy.get("[data-cy=availability-save-failure]")
      .find('[role="button"][aria-pressed="false"]')
      .first()
      .click();
    cy.get("[data-cy=availability-save-failure]")
      .contains("button", "Lagre tilgjengelighet")
      .click();

    cy.get("[data-cy=availability-save-failure]")
      .should("contain.text", "Endringer klare til lagring")
      .contains("button", "Lagre tilgjengelighet")
      .should("be.enabled");
    cy.get<unknown[]>("@unhandledRejections").should("have.length", 0);
  });

  it("keeps experience changes retryable without an unhandled rejection", () => {
    collectUnhandledRejections().as("unhandledRejections");
    cy.get("[data-cy=availability-experience-failure]")
      .contains("button", "Administrer")
      .click();
    cy.get(
      '[data-cy=availability-experience-failure] [aria-label="Erfaringsnivå for Ane Solberg"]',
    ).click();
    cy.contains('[role="option"]', "Erfaren").click();

    cy.get(
      '[data-cy=availability-experience-failure] [aria-label="Erfaringsnivå for Ane Solberg"]',
    )
      .should("be.enabled")
      .and("contain.text", "Ikke klassifisert");
    cy.get<unknown[]>("@unhandledRejections").should("have.length", 0);
  });
  ...
  it("does not steal focus from a deliberate outside target", () => {
    cy.get(
      ...
      .focus()
      .type("{enter}");
    cy.get("[data-cy=schedule-slot-editor]").should("be.visible");

    cy.get(
      '[data-cy=admin-config-harness] input[aria-label="Starttid per dag, time"]',
    )
      .as("outsideTimeInput")
      .click({ force: true });
    cy.get("[data-cy=schedule-slot-editor]").should("not.exist");
    ...
    cy.get("@outsideTimeInput").should("be.focused");
  });
  ...
  it("restores the current whole-block baseline after fine-tuning", () => {
    cy.get("[data-cy=admin-config-harness]").within(() => {
      ...
      cy.get(
        '[role="radiogroup"][aria-label="Redigeringsnivå for intervjutider"]',
      )
        .contains("button", "Standardblokker")
        .click()

...
import { clickCalendarDates } from "../support/calendar";
...
const mountSettings = (
  ...
  visitStaticFixture("admin-schedule-settings-popover", {
    value: initialPause,
    custom: initialIsCustom ? "1" : "0",
...
const chooseInterviewPeriod = (startDate: string, endDate: string) => {
  ...
  clickCalendarDates(startDate, endDate);
...
describe("inline schedule settings and standard-block preview", () => {
  ...
  it("uses the same expand-contract motion for pauses and final-block actions", () => {
    ...
    cy.window()
      .then((window) => {
        const pauseOption = window.document.querySelector<HTMLInputElement>(
          '[role="radiogroup"][aria-label="Pause mellom blokker"] input[type="radio"][value="60"]',
        );
        if (!pauseOption) throw new Error("Pause option was not rendered");

        const framesPromise = new Cypress.Promise<
          { pause: number; actions: number }[]
        >((resolve) => {
        ...
        // Start the animation in this browser task so frame sampling cannot
        // be delayed until after the transition by Cypress command retries.
        pauseOption.click();
        return framesPromise;
      })
      .then((frames) => {
        expect(Math.min(...frames.map((frame) => frame.pause))).to.be.lessThan(
          0.95,
        );
        expect(
          Math.min(...frames.map((frame) => frame.actions)),
        ).to.be.lessThan(0.95);
        expect(
          Math.max(
  ...
  it("updates period and daily time without an intermediate save", () => {
    ...
    cy.get('input[aria-label="Starttid per dag, time"]')
      .as("startHour")
      .focus();
    cy.window().then(
      (window) =>
    ...
    cy.get("@startHour").should("be.focused").clear().type("9");
  ...
  it("exposes a valid row hierarchy for the calendar grid", () => {
    mountSettings();
    cy.get("[data-cy=interview-period-trigger]").click();
    cy.get('[role="grid"]').then(($grid) => {
      cy.wrap($grid).children('[role="row"]').should("have.length.at.least", 5);
      cy.wrap($grid)
        .children('[role="row"]')
        .first()
      ...
      cy.wrap($grid)
        ...
        .not(":first")
        .find('[role="gridcell"]')
        .should("have.length.at.least", 35);
    });
  });
  ...
  it("supports keyboard selection across month boundaries", () => {
    ...
    cy.get('[data-calendar-date="2026-08-01"]')
      ...
      .focus();
    cy.focused().type("{rightarrow}".repeat(8), { delay: 0 });
    cy.get(
      '[data-cy="interview-period-dialog"] [data-calendar-date][tabindex="0"]',

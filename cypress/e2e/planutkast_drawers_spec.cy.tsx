...
const RejectedConflictReviewHarness = () =>
  h(ConflictReviewView, {
    candidates,
    currentParticipant: participant,
    onSaveReview: async () => {
      throw new Error("Fixture conflict review failure");
    },
    openRequestKey: 1,
    showSummary: false,
  });
...
describe("focused Planutkast interactions", () => {
  ...
  it("keeps a rejected candidate review open without an unhandled rejection", () => {
    mountInPage(h(RejectedConflictReviewHarness));
    ...
    cy.get("[data-cy=conflict-candidate-candidate-1]").should("be.checked");
    cy.get("[data-cy=conflict-submit]").should("be.enabled");
  ...
  it("uses the opened collection snapshot and requires every participant before closing", () => {
    ...
    cy.get("[data-cy=toggle-conflict-collection]")
      .should("be.disabled")
      .and("contain.text", "Fullfør kontrollen");
  ...
  it("lets an admin close a stale collection before reopening it", () => {
    ...
    cy.get("[data-cy=toggle-conflict-collection]")
      .should("be.enabled")
      .and("contain.text", "Lukk utdatert kontroll");
